...
Kalshi trading volume is the total number of contracts traded across all markets over a given period. If you searched for [what does volume mean on Kalshi](/guides/what-does-volume-mean-on-kalshi), start with that explainer for definitions, liquidity, and open interest — this guide focuses on trends and quarterly data.
...
For a plain-language breakdown of [what volume means on Kalshi](/guides/what-does-volume-mean-on-kalshi) — including profile volume, liquidity, and how volume differs from probability — see the dedicated explainer. At its core, [Kalshi volume meaning](/guides/what-does-volume-mean-on-kalshi) is simple:
...
## Explore Kalshi volume data yourself

Want to inspect the trade-level data behind Kalshi volume trends? Build a query on historical Kalshi trades — aggregate by time, filter by category, or isolate event-driven spikes — and run it instantly.

<KalshiHistoricalDataQuery
  title="Query Kalshi historical volume data"
  description="Pull Kalshi trade history, select volume-related columns, and explore the same dataset used in the charts above."
/>
