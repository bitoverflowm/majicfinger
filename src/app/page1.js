...
export default function Home() {
  ...
  */
  ...
  const [formattedData, setFormattedData] = useState(defaultBarData)
  const [graphKeys, setGraphKeys] = useState(["category"])
  const [yAxisName, setYAxisName] = useState("Value")
  ...
  return (
    <div className='w-full bg-white flex flex-col place-content-center place-items-center'>
      ...
      <div className='absolute top-10 left-10 flex flex-col z-20'>
        ...
        <div>
          <div className='text-xxs py-1'>Pick Color Scheme</div>
          <div className='flex flex-wrap gap-2'>
            <div className={`${button} ${undBtn}`} onClick={()=>setColorScheme('nivo')}>Nivo</div>
            <div className={`${button} ${undBtn}`} onClick={()=>setColorScheme('category10')}>Category10</div>
            <div className={`${button} ${undBtn}`} onClick={()=>setColorScheme('accent')}>Accent</div>
            <div className={`${button} ${undBtn}`} onClick={()=>setColorScheme('dark2')}>Dark2</div>
          </div>
          <div className='flex text-xs place-content-center place-items-center pt-3'>
            <div onClick={()=> borderRadius > 0 && setBorderRadius(borderRadius-5)} className="h-6 text-md cursor-pointer hover:bg-black hover:text-white border border-black rounded-l-lg py-1 px-1"><IoMdArrowDropdown /></div>
            <div className='h-6 border border-black text-xxs px-2 py-1'>Border Radius</div>
            <div className="h-6 text-md cursor-pointer hover:bg-black hover:text-white border border-black rounded-r-lg py-1 px-1" onClick={()=>setBorderRadius(borderRadius+5)}><IoMdArrowDropup /></div>
          </div>
          <div className='text-xxs flex gap-2 my-2 place-items-center'>
            <div>X-Axis</div>
            <div>
              <textarea className="h-6 bg-white rounded-lg text-center py-1 shadow-inner resize-none border border-black" type="text" placeholder="new x-axis" value={xName} onChange={e => setXName(e.target.value)} maxLength={50}/>
            </div>
      ...
      <div className={`w-screen`}>
        ...
        <div className={'w-full h-screen flex place-items-center place-content-center'}>
          <div className='w-7/12 min-h-[600px] p-10 bg-majic-white shadow-xl rounded-lg text-center flex flex-col place-content-center'>
              {
                chart && chart === "ai"
                  && <div className='text-xs flex flex-col'>
                      ...
                      <div className='flex flex-cols plaace-items-center place-content-center'>
                        ...
                        {
                          generatedData && <div><CsvToHtmlTable data={generatedData && generatedData} csvDelimiter="," tableClassName="table table-striped table-hover"/></div>
                        }
                        { reccommendedCharts &&
                        ...
                        {
                          graphReady && 
                            <div className='w-[700px] h-[600px] flex place-items-center place-content-center'>
