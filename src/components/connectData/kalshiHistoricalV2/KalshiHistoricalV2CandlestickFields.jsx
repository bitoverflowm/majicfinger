...
function genFilterId(column) {
  return `khv2-cw-${column}-${Date.now().toString(36)}`;
}
...
function formatRangeLabel(from, to) {
  ...
  if (a) return `${a} – …`;
  if (b) return `… – ${b}`;
  return "Pick start and end dates (required)";
}

/**
 * Historical v2 candlesticks: multi-ticker search + required date range (≤ cutoff) + period.
 *
 * @param {{ className?: string; disabled?: boolean }} props
 */
export function KalshiHistoricalV2CandlestickFields({ className, disabled = false }) {
  ...
  const { cutoffIso, cutoffMs } = useKalshiHistoricalCutoffDisplay();
  ...
  // Seed period + a default window ending at the historical cutoff once known.
  useEffect(() => {
    if (!setConnectKalshiLiveWhereFilters || seededDefaults) return;
    const cutoffSec =
      cutoffMs != null && Number.isFinite(Number(cutoffMs))
        ? Math.floor(Number(cutoffMs) / 1000)
        : null;
    ...
    setConnectKalshiLiveWhereFilters((prev) => {
      ...
      if (!Number.isFinite(Number(readFilterValue(next, "end_ts")))) {
        next = upsertApiFilter(next, "end_ts", cutoffSec);
        changed = true;
      }
      if (!Number.isFinite(Number(readFilterValue(next, "start_ts")))) {
        next = upsertApiFilter(next, "start_ts", Math.max(0, cutoffSec - DEFAULT_RANGE_SEC));
        changed = true;
      }
      return changed ? next : prev;
    });
    setSeededDefaults(true);
  }, [cutoffMs, seededDefaults, setConnectKalshiLiveWhereFilters]);

  // Clamp end (and start) to cutoff — historical cannot go past Live Data Feed stop.
  useEffect(() => {
    if (!cutoffDate || !setConnectKalshiLiveWhereFilters) return;
    const maxSec = unixFromDate(endOfLocalDay(cutoffDate));
    if (maxSec == null) return;
    setConnectKalshiLiveWhereFilters((prev) => {
      let next = Array.isArray(prev) ? prev : [];
      let changed = false;
      const end = Number(readFilterValue(next, "end_ts"));
      const start = Number(readFilterValue(next, "start_ts"));
      if (Number.isFinite(end) && end > maxSec) {
        next = upsertApiFilter(next, "end_ts", maxSec);
        changed = true;
      }
      if (Number.isFinite(start) && start > maxSec) {
        next = upsertApiFilter(next, "start_ts", maxSec);
        changed = true;
      }
      return changed ? next : prev;
    });
  }, [cutoffDate, endTs, startTs, setConnectKalshiLiveWhereFilters]);

  // Keep window under Kalshi's 5000-candle cap.
  useEffect(() => {
  ...
  const commitRange = useCallback(
    (from, to) => {
      ...
      if (cutoffDate) {
        if (fromDay && fromDay > cutoffDate) fromDay = new Date(cutoffDate);
        if (toDay && toDay > cutoffDate) toDay = new Date(cutoffDate);
      }
    ...
    <div className={cn("space-y-4", className)}>
      <div className="space-y-2">
        ...
        <p className="text-[11px] leading-snug text-muted-foreground">
          Search up to 100 markets. Each market is queried separately and lands in its own sheet
          (max 10,000 rows per market). Semantic search helps confirm the ticker exists.
        </p>
        <div className="space-y-2 rounded-lg bg-muted/10 p-3">
          <MarketTickerSearch
            ...
            onSelectionsChange={(selections) => {
              ...
              setConnectKalshiLiveCandlestickTickerMeta?.(next);
            }}
          />
        </div>
      </div>
      ...
      <div className="space-y-2">
        ...
        <p className="text-[11px] leading-snug text-muted-foreground">
          Date range is required and maps to{" "}
          <span className="font-mono text-[10px]">start_ts</span> /{" "}
          <span className="font-mono text-[10px]">end_ts</span>. Dates must be on or before the
          Kalshi Live Data Feed cutoff.
        </p>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-[10px] leading-snug text-muted-foreground">
              Finer intervals allow a shorter date span (Kalshi max 5,000 candles per request).
            </p>
          </div>
        </div>
      </div>
