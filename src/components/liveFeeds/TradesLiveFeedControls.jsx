...
import { discoverTradesFeedGroup } from "@/lib/liveFeeds/feedConfig";
...
/**
 * Start / pause / stop live REST poll for Kalshi market trades.
 */
export function TradesLiveFeedControls() {
  ...
  const group = useMemo(() => discoverTradesFeedGroup(dataSheets), [dataSheets]);
  ...
  const activeFeed = useMemo(() => {
    ...
    return (
      feeds.find((f) => {
        if (f?.integration !== "kalshi-live" || f?.endpoint !== "trades") return false;
    ...
    setPollDraftMs(
      clampLiveFeedPollIntervalMsForEndpoint(
        activeFeed?.pollIntervalMs ??
          (savedLiveSource?.endpoint === "trades" ? savedLiveSource.pollIntervalMs : null) ??
          60_000,
        "kalshi-live",
        "trades",
      ),
  ...
  const pollValueMs = useMemo(() => {
    ...
    return clampLiveFeedPollIntervalMsForEndpoint(pollDraftMs, "kalshi-live", "trades");
  }, [isRunning, activeFeed?.pollIntervalMs, pollDraftMs]);

  const handleStart = useCallback(() => {
    const result = startTradesEditorLiveFeed({
      dataSheets,
      liveFeedActions,
      liveFeedState,
    ...
    if (!result.started && result.skipped && result.skipped !== "already_running") {
      toast.message(
        result.skipped === "markets_closed"
          ? "Markets are closed — live trades feed unavailable"
          : "Could not start trades live feed",
      );
    }
  }, [
    dataSheets,
  ...
  const handlePause = useCallback(() => {
    ...
    toast.message("Live trades feed paused");
  }, [activeFeed?.id, liveFeedActions]);

  const handleResume = useCallback(() => {
    if (!activeFeed?.id) return;
    liveFeedActions?.resume?.(activeFeed.id);
    toast.success("Live trades feed resumed");
  }, [activeFeed?.id, liveFeedActions]);

  const handleStop = useCallback(() => {
    if (!activeFeed?.id) return;
    liveFeedActions?.stop?.(activeFeed.id);
    toast.message("Live trades feed stopped");
  }, [activeFeed?.id, liveFeedActions]);
  ...
  return (
    <div className="space-y-2 rounded-md border border-border/60 bg-muted/20 p-2">
      <div className="flex items-center gap-1.5">
        ...
        <p className="text-xs font-semibold text-foreground">Live trades feed</p>
      </div>
      ...
      {isRunning && activeFeed?.lastTickStats ? (
        <p className="text-[10px] leading-snug text-muted-foreground tabular-nums">
          ...
          {" · "}
          {activeFeed.lastTickStats.tradesReceived} trades
          {activeFeed.lastTickStats.tradesAdded > 0
            ? ` · +${activeFeed.lastTickStats.tradesAdded} new`
            : ""}
        </p>
      ) : null}
      ...
      {!isRunning ? (
        <>
          ...
          >
            ...
            <span className="min-w-0">Enable live trades feed</span>
          </Button>
        </>
      ) : (
