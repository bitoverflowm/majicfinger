...
export function LineChart({
  ...
  const createSmoothPath = (points: { x: number; y: number }[]) => {
    ...
    return points.reduce((acc, point, i, arr) => {
      ...
      const prev = arr[i - 1];
      const next = arr[i + 1];
      if (i === arr.length - 1) return `${acc} L ${point.x} ${point.y}`;
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setShowPulse(true), ((startAnimationDelay || 0) * 1000) | 0);
    return () => clearTimeout(t);
  }, [shouldAnimate, startAnimationDelay]);

  const [computedColor, setComputedColor] = useState(color);
  useEffect(() => setComputedColor(getRGBA(color)), [color]);
...
function NumberFlowCounter({
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => setShowCounter(true), ((startAnimationDelay || 0) * 1000) | 0);
    return () => clearTimeout(t);
  }, [shouldAnimate, startAnimationDelay]);
  ...
  return (
    ...
    >
      <NumberFlow
        ...
        format={{ useGrouping: true, minimumIntegerDigits: 1 }}
      />
    </div>
  );
}
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="absolute top-[60%] left-1/2 -translate-x-1/2 w-[2px] h-32 bg-gradient-to-b from-[var(--color)] to-transparent z-10"
      />
      <NumberFlowCounter
