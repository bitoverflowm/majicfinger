...
export const Feature = ({
  ...
  const scrollToIndex = useCallback((index: number) => {
    ...
    if (!el) return;
    const card = el.querySelectorAll(".card")[index];
    if (card) {
  ...
  }, [isInView, collapseDelay, featureItems.length]);
  ...
  const accordionValue = currentIndex >= 0 ? `item-${currentIndex}` : "";
          ...
          >
            {featureItems.map((item, index) => (
              <button
                key={item.id}
                type="button"
                className="card relative grid h-full max-w-64 shrink-0 items-start justify-center p-3 bg-background border-l last:border-r border-t border-b first:rounded-tl-xl last:rounded-tr-xl text-left"
