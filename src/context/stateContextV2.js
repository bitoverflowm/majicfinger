...
export const StateProviderV2 = ({children, initialSettings}) => {
    ...
    /** Kalshi Live trades: single market ticker (required). */
    const [connectKalshiLiveTradesTicker, setConnectKalshiLiveTradesTicker] = useState("");
    /** Kalshi Live GET /series/{series_ticker} path param. */
    const [connectKalshiLiveSeriesTicker, setConnectKalshiLiveSeriesTicker] = useState("");
    /** Kalshi Live GET /series list filters (category, tags, min_updated_ts). Legacy — prefer whereFilters. */
    const [connectKalshiLiveSeriesListFilters, setConnectKalshiLiveSeriesListFilters] = useState([]);
    /** Kalshi Live series list client-side sort id. Legacy — prefer sortClauses. */
    const [connectKalshiLiveSeriesListSort, setConnectKalshiLiveSeriesListSort] = useState("");
    ...
    const requestConnectWorkspace = useCallback((id, options) => {
      if (!id) {
        ...
        setConnectKalshiLiveSeriesListFilters([]);
        setConnectKalshiLiveSeriesListSort("");
