/**
 * Fetch a single Kalshi Historical v2 market by ticker (via same-origin proxy).
 *
 * @param {{ marketTicker: string; signal?: AbortSignal }} opts
 * @returns {Promise<Record<string, unknown>>}
 */
export async function fetchKalshiHistoricalV2Market(opts) {
  ...
  const res = await fetch(`/api/integrations/kalshi-live/historical/markets/get?${qs.toString()}`, {
