...
import { KALSHI_LIVE_SEARCH_TRADERS_COLUMNS } from "@/lib/kalshiLive/searchTradersColumns";
import {
...
import { KALSHI_LIVE_EVENTS_COLUMNS, KALSHI_LIVE_MULTIVARIATE_EVENTS_COLUMNS } from "@/lib/kalshiLive/eventsColumns";
...
export function getKalshiLiveAllColumnNames(endpointId) {
  ...
  if (endpointId === "holder_profile") {
    return KALSHI_LIVE_HOLDER_PROFILE_COLUMNS.map((c) => c.name);
  }
  if (endpointId === "trades_by_holder") {
    return KALSHI_LIVE_HOLDER_TRADES_COLUMNS.map((c) => c.name);
  }
  if (endpointId === "search_traders") {
    return KALSHI_LIVE_SEARCH_TRADERS_COLUMNS.map((c) => c.name);
  }
...
export function getKalshiLiveColumnType(endpointId, column) {
  ...
  if (endpointId === "holder_profile") {
    const row = KALSHI_LIVE_HOLDER_PROFILE_COLUMNS.find((c) => c.name === column);
    return row?.type || "string";
  }
  if (endpointId === "trades_by_holder") {
    const row = KALSHI_LIVE_HOLDER_TRADES_COLUMNS.find((c) => c.name === column);
    return row?.type || "string";
  }
  if (endpointId === "search_traders") {
    const row = KALSHI_LIVE_SEARCH_TRADERS_COLUMNS.find((c) => c.name === column);
    return row?.type || "string";
  }
  if (endpointId === "trades") {
...
export function validateKalshiLiveWhereFilters(endpointId, whereFilters) {
  ...
  if (endpointId === "series" || endpointId === "events" || endpointId === "multivariate_events") {
