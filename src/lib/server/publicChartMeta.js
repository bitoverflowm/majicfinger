 ...
 * @param {string} slug
 * @returns {Promise<{ chart_name?: string, og_image_url?: string, has_og_image_data?: boolean } | null>}
 */
export async function getPublicChartMeta(username, slug) {
  ...
  const chart = await Chart.findOne({
    ...
    .select("chart_name og_image_url og_image_data")
    .lean();
  if (!chart) return null;
  return {
    chart_name: chart.chart_name || "Chart",
    og_image_url: chart.og_image_url || undefined,
    has_og_image_data: !!chart.og_image_data,
  };
}
