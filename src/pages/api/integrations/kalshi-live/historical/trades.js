...
/**
 * Proxy GET /historical/trades.
 * ticker, min_ts, max_ts, is_block_trade are optional; limit defaults to 1000 (API max).
 */
export default async function handler(req, res) {
  ...
  const blockRaw = queryParam(req, "is_block_trade").toLowerCase();
  ...
  if (blockRaw === "true" || blockRaw === "1") qs.set("is_block_trade", "true");
  if (blockRaw === "false" || blockRaw === "0") qs.set("is_block_trade", "false");

  const url = `${kalshiLiveUrl("historical/trades")}?${qs.toString()}`;
  ...
  try {
        ...
        error:
          typeof body?.message === "string"
            ...
            : typeof body?.error === "string"
              ? body.error
              : upstream.statusText || "Kalshi historical trades request failed",
        ...body,
      });
    }
    ...
    return res.status(502).json({
      error: e instanceof Error ? e.message : "Failed to reach Kalshi historical trades API",
    });
  }
}
