...
/**
 * Proxy GET /v1/social/profile (elections social API host).
 */
export default async function handler(req, res) {
  ...
  const url = `${kalshiLiveSocialUrl("social/profile")}?${qs.toString()}`;
  ...
  try {
          ...
          typeof nested === "string"
            ...
            : typeof body?.message === "string"
              ...
              : typeof body?.error === "string"
                ? body.error
                : upstream.statusText || "Kalshi holder profile request failed",
    ...
    return res.status(502).json({
      error: e instanceof Error ? e.message : "Failed to reach Kalshi social profile API",
    });
  }
}
